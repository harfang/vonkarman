...
class VonKarman():
	...
	def __init__(self, Lx = 30.0, Ly = 7.0, Nx = 150, Ny=35, Nt=1200, r = 0.35, Re = 100, pas_enregistrement = 5):
		...
		self.ustar = np.zeros((Ny,Nx))
		self.vstar = np.zeros((Ny,Nx))
	...
	def create_dir(self):
		...
		self.color_norm_w = matplotlib.colors.Normalize(vmin=-10,vmax=10)
	...
	def condition_cfl(self):
		...
		dt_adv = facteur_de_precaution_cfl * min(self.dx, self.dy)/max(u_max, v_max, 0.001)
	...
	def cl_soufflerie(self):
		...
		self.ustar[:, :2] = 1 #la vitesse est de 1 en entrée
		...
		self.ustar[-1,1:-1]= self.ustar[-3,1:-1] #en haut la contrainte est nulle
	...
	def points_fantome_vitesse(self):
		self.u[:, 0] = 1 #la vitesse est de 1 en entrée
		self.v[:, 0] = 0 #la vitesse v est nulle en entrée
		self.u[0,:]= self.u[2,:] #en haut la contrainte est nulle
		self.v[0, :] = 0 #en haut, la vitesse normale est nulle
		self.u[-1,:]= self.u[-3,:] #en haut la contrainte est nulle
		self.v[-1, :] = 0 #en bas, la vitesse normale est nulle
		self.u[:, self.Nx-1] = self.u[:, self.Nx-3] #dérivée de u par rapport à x nulle à la sortie
		self.v[:, self.Nx-1] = self.v[:, self.Nx-3] #dérivée de v par rapport à x nulle à la sortie
	...
	def main_loop(self):
		...
		for n in range(self.Nt):
			...
			if n%self.pas_enregistrement == 0:
				...
				plt.imshow(np.sqrt(self.u[1:-1,1:-1]**2+self.v[1:-1,1:-1]**2), origin='lower', cmap='hot', interpolation = 'none') #norm = color_norm
				...
				plt.axis('image')
				if n<10:		
					plt.savefig("image000{}_t={}_Re={}.jpg".format(n+1, t_simu, self.Re))
				elif n<100:
					plt.savefig("image00{}_t={}_Re={}.jpg".format(n+1, t_simu, self.Re))
				elif n<1000:
					plt.savefig("image0{}_t={}_Re={}.jpg".format(n+1, t_simu, self.Re))
				else:
					plt.savefig("image{}_t={}_Re={}.jpg".format(n+1, t_simu, self.Re))
		...
		print(np.sqrt(self.u[1:-1,1:-1]**2+self.v[1:-1,1:-1]**2))

simu = VonKarman(Re = 10, Nt = 1200, Nx = 75*2, Ny = 35)
simu.main_loop()
